...
const getSingleMentor = catchAsync(async (req: Request, res: Response) => {
  ...
  const result = await MentorService.getSingleMentor(id, mentee as string);
...
const getMenteeByMentor = catchAsync(async (req: Request, res: Response) => {
  ...
  const result = await MentorService.getMenteeByMentor(
    req.user,
    paginationOptions,
    filters
  );
