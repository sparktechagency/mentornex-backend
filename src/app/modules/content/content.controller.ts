...
const addContent = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
...
const updateContent = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
  const { id } = req.params;
  const payload = req.body;
  const user = req.user;
  payload.mentor = user.id;
...
const deleteContent = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
  const { id } = req.params;
  const user = req.user;
...
export const ContentController = { addContent, updateContent, deleteContent, getAllContent };
